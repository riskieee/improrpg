...
.album.py-5
  .container
    ...
    .row.row-cols-1.row-cols-sm-2.row-cols-md-3.g-3
      .col
        .card.shadow-sm
          img.card-img-top(width='100%' height='225' :src="`../assets/${story.storyCover}`" alt='Storycover' aria-label='Storycover' preserveaspectratio='xMidYMid slice' focusable='false')
          .card-body
            p.card-text
            p {{ story.storyCover }}
              h5 {{ story.storyName }}
            ...
            .d-flex.justify-content-between.align-items-center
              .btn-group
                router-link.btn.btn-sm.btn-outline-primary.px-3(type='button' :to="`/storys/${story._id}`") Read
